...
Then, given a dataset $\mathcal{D}=\{\theta_i,x_i\}_{i=1}^{N}$ of $N$ i.i.d parameter-measurement pairs, $\mathcal{D}$ of size $N$, lower bound the mean square error of $\hat{\theta}(\cdot)$ by a learned bound ${LB}(\mathcal{D})$, satisfying:
$$\textbf{non-Bayesian: }\mathbb{E}[\epsilon\epsilon^T|\theta]\succeq {B}(\theta)\xleftarrow{\substack{\infty\leftarrow{}N}}\matsym{LB}(\theta;\mathcal{D})\quad \text{a.s},$$
...
Paper: https://ieeexplore.ieee.org/document/9827830
...
Paper: https://ieeexplore.ieee.org/document/10446725
...
Paper: https://arxiv.org/abs/2502.00724
